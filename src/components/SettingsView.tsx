...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const handleSyncIntegration = (intId: string, name: string) => {
    setSyncingIntegrationId(intId);
    setTimeout(() => {
      setSyncingIntegrationId(null);
      showToast(`Sincronização com ${name} concluída com sucesso!`);
    }, 1200);
  };
                ...
                {integrations.map((int) => (
                  ...
                  >
                    ...
                    <div className="pt-2 border-t border-gray-100 flex items-center justify-between text-[11px]">
                      <span className="text-gray-500">{int.statusText}</span>
                      <button
                        type="button"
                        onClick={() => handleSyncIntegration(int.id, int.name)}
                        disabled={!int.connected || syncingIntegrationId === int.id}
                        className="px-2.5 py-1 rounded-lg bg-gray-50 hover:bg-[#eff4ff] text-[#0051d5] font-semibold flex items-center gap-1 disabled:opacity-50 transition-colors"
                      >
                        <span
